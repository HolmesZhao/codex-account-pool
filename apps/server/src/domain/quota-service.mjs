...
export class QuotaService {
  constructor({ repository, permissionService, runtime, vault, now = () => new Date() }) { this.repository = repository; this.permissionService = permissionService; this.runtime = runtime; this.vault = vault; this.now = now; }
  async refresh(subject, accountId) {
    ...
    try {
      ...
      const current = await this.runtime.readQuota(JSON.stringify(auth));
